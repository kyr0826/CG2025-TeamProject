...
private:  
	...
	void InitFruitData() {  
		...
		fruitTypes.push_back({ "Watermelon", 10, 1.60f, glm::vec3(0.15f, 0.7f, 0.2f), 35.0f });		// 10. 수박


		//fruitTypes.push_back({ "Blueberry",		0, 0.15f, glm::vec3(0.8f, 0.0f, 0.0f), 1.0f });		// 0. 블루베리  
		//fruitTypes.push_back({ "Cherry",		1, 0.23f, glm::vec3(1.0f, 0.2f, 0.2f), 2.0f });		// 1. 체리
		//fruitTypes.push_back({ "Plum",			2, 0.32f, glm::vec3(0.6f, 0.2f, 0.8f), 3.0f });		// 2. 자두  
		//fruitTypes.push_back({ "Mandarin",		3, 0.40f, glm::vec3(1.0f, 0.6f, 0.0f), 4.0f });		// 3. 귤
		...
		//fruitTypes.push_back({ "Pear",			6, 0.75f, glm::vec3(0.9f, 0.9f, 0.6f), 10.0f });	// 6. 배
		//fruitTypes.push_back({ "Peach",			7, 0.90f, glm::vec3(1.0f, 0.7f, 0.8f), 13.0f });	// 7. 복숭아  
		//fruitTypes.push_back({ "Papaya",		8, 1.10f, glm::vec3(0.9f, 0.8f, 0.2f), 18.0f });	// 8. 파파야
